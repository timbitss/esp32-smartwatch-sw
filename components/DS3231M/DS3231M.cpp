/*!
 @file DS3231M.cpp
 @section Arduino Library to access the Maxim DS3231M Real Time Clock.
...
/** 
 * @brief     Returns the number of days from a given Y M D value since 2000-01-01.
 * @param[in] y Years.
 * @param[in] m Months [1, 12].
 * @param[in] d Days.
...
static uint16_t date2days(uint16_t y, uint8_t m, uint8_t d)
{
    ...
    {
        days += days_in_month[i - 1]; // Add number of days for each month.
    }

    if (m > 2 && y % 4 == 0)
    {
        ++days; // Add a day for leap years.
    }
...
DateTime::DateTime(const char *date, const char *time)
{
  ...
  */
    ...
    memcpy(ybuff, date, 11);   // copy string to memory.
    memcpy(tbuff, time, 9);    // copy string to memory.
